...
if __name__ == "__main__":
    ...
    raw_data_filename = 'org.nii.gz' # original
    ...
    for sub in subs:
        ...
        mask_volume = np.zeros(image_size)  # (82, 92, 56)
        mask_volume[:np.minimum(mask_handle.shape[0], image_size[0], dtype=int),
        :np.minimum(mask_handle.shape[1], image_size[1], dtype=int),
        :np.minimum(mask_handle.shape[2], image_size[2], dtype=int)] = mask_handle.get_fdata()[:np.minimum(mask_handle.shape[0], image_size[0], dtype=int), :np.minimum(mask_handle.shape[1], image_size[1], dtype=int), :np.minimum(mask_handle.shape[2], image_size[2], dtype=int)]  #
        for proc_path in proc_paths:
            print(proc_path)
            mri_handle = nib.load(filename=proc_path)

            mri_volume = np.zeros(image_size) # (82, 92, 56)
            mri_volume[:mri_handle.shape[0], :mri_handle.shape[1], :mri_handle.shape[2]] = mri_handle.get_fdata()
            mri_volume = mri_volume * mask_volume # apply mask to volume
