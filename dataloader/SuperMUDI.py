...
def DefineTrainValSuperMudiDataloader_moe(gen_conf, train_conf, is_shuffle_trainval = False):
    ...
    trainDataloader = SuperMudiSequence_moe(gen_conf, train_conf, shuffle_order[:train_volumes], N_train)
    if validation_split_ratio != 0:

        # shuffle_order is False, so 32000-4000 patch for val
        valDataloader = SuperMudiSequence_moe(gen_conf, train_conf, shuffle_order[train_volumes:], N_val)
    else:
        valDataloader = None

    return trainDataloader, valDataloader

class SuperMudiSequence_moe(Sequence):
